...
void scatti_profile_init(CRuckigProfile *p) {
    ...
    scatti_brake_init(&p->brake);
    scatti_brake_init(&p->accel);
...
bool scatti_profile_check(CRuckigProfile *p, CRuckigControlSigns cs, CRuckigReachedLimits lim,
    ...
    if (lim == ReachedLimitsACC0_ACC1_VEL || lim == ReachedLimitsACC0_VEL || lim == ReachedLimitsACC1_VEL || lim == ReachedLimitsVEL) {
        if (SCATTI_UNLIKELY(p->t[3] < DBL_EPSILON)) {
            return false;
        }
    }

    if (lim == ReachedLimitsACC0 || lim == ReachedLimitsACC0_ACC1) {
        if (SCATTI_UNLIKELY(p->t[1] < DBL_EPSILON)) {
            return false;
        }
    }

    if (lim == ReachedLimitsACC1 || lim == ReachedLimitsACC0_ACC1) {
        if (SCATTI_UNLIKELY(p->t[5] < DBL_EPSILON)) {
            return false;
        }
    }

    if (SCATTI_UNLIKELY(p->t_sum[6] > PROFILE_T_MAX)) {
        return false;
    }
...
bool scatti_profile_check_for_second_order_with_timing_full(CRuckigProfile *p, CRuckigControlSigns cs, CRuckigReachedLimits lim,
    ...
    return (aMin - PROFILE_A_EPS < aUp) && (aUp < aMax + PROFILE_A_EPS) && (aMin - PROFILE_A_EPS < aDown) && (aDown < aMax + PROFILE_A_EPS)
        && scatti_profile_check_for_second_order_with_timing(p, cs, lim, tf, aUp, aDown, vMax, vMin);
}
